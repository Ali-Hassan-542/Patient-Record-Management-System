                            ...
                            <?php if ($result->num_rows > 0) : ?>
                                <?php while ($row = $result->fetch_assoc()) : ?>
                                    <tr>
                                        ...
                                        <td>
                                            <img src="uploads/<?= !empty($row['image']) ? $row['image'] : 'default1.png' ?>" alt="Patient Image" style="width:100px; height:100px; object-fit:cover;">
                                        </td>
                                        <td>
                                            <!-- <a href="print_report.php?patient=<?= urlencode($row['patient_name']) ?>&doctor=<?= urlencode($row['doctor_name']) ?>&medication=<?= urlencode($row['medication_name']) ?>&status=<?= urlencode($row['status']) ?>&date=<?= urlencode($row['created_at']) ?>"
                                                target="_blank"
                                                class="btn btn-sm btn-outline-primary">
                                                <i class="fas fa-print"></i> Print
                                            </a> -->
                                            <a href="print_report.php?patient=<?= urlencode($row['patient_name']) ?>&doctor=<?= urlencode($row['doctor_name']) ?>&medication=<?= urlencode($row['medication_name']) ?>&status=<?= urlencode($row['status']) ?>&date=<?= urlencode($row['created_at']) ?>&image=<?= urlencode($row['image'] ?? 'default1.png') ?>"
                                                target="_blank"
                                                class="btn btn-sm btn-outline-primary">
                                                <i class="fas fa-print"></i> Print
                                            </a>
