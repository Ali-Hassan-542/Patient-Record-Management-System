...
// Fetch Patients List
$patients = $conn->query("SELECT id, name FROM patients WHERE id NOT IN (SELECT DISTINCT patient_id FROM medications) ORDER BY name ASC");
...
// Fetch Medications
$medications = $conn->query("SELECT medications.*, patients.name AS patient_name, doctors.name AS doctor_name FROM medications JOIN patients ON medications.patient_id = patients.id JOIN doctors ON medications.prescribed_by = doctors.id ORDER BY medications.created_at DESC");
...
if (isset($_POST['add_medication'])) {
    ...
    $query = "INSERT INTO medications (patient_id, medication_name, dosage, frequency, prescribed_by, start_date, end_date, status, document, created_at) VALUES ('$patient_id', '$medication_name', '$dosage', '$frequency', '$doctor_id', " . ($start_date ? "'$start_date'" : "NULL") . ", " . ($end_date ? "'$end_date'" : "NULL") . ", '$status', '$document', NOW())";
...
if (isset($_POST['update_medication'])) {
    ...
    // update query with document
    $query = "UPDATE medications SET medication_name='$medication_name', dosage='$dosage', frequency='$frequency', prescribed_by='$doctor_id', start_date=" . ($start_date ? "'$start_date'" : "NULL") . ", end_date=" . ($end_date ? "'$end_date'" : "NULL") . ", status='$status'" . ($document ? ", document='$document'" : "") . " WHERE id='$id'";
                            ...
                            <?php if ($medications->num_rows > 0): ?>
                                <?php while ($med = $medications->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            $doc = !empty($med['document']) ? $med['document'] : 'default1.png'; // NEW CODED
                                            ?>
                                            <img src="uploads/<?= $doc ?>" class="img-thumbnail rounded" style="width: 80px; height: 80px; object-fit: cover;" alt="Document">
                                        </td>
            ...
            <div class="modal-content">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <img id="edit_current_document" src="" alt="Document" style="width: 100px; height: auto; border:1px solid #ccc;" />
                    </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        document.querySelectorAll(".edit-btn").forEach(button => {
            button.addEventListener("click", function() {
                ...
                let docPath = "uploads/" + this.dataset.document;
                document.getElementById("edit_current_document").src = docPath;
